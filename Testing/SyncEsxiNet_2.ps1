...
{
 if ($DVss -notcontains $vss) #checks if the destination host already has this vswitch and if not then proceeds to creates the vswitch
 { get-vmhost $DHost | New-VirtualSwitch -Name $vss -Confirm:$false } #creates vSwtch
}
 
#Add vmnics to vSwitchs
...
#Replicate VMkernels
$vmks = (get-vmhost $DHost | Get-VMHostNetworkAdapter -VMKernel).DeviceName
...
#NIC Teaming Policies for vswitchs
 foreach ($vss in (get-vmhost $SHost | Get-virtualswitch))
 {
...
#NIC Teaming Policies for portgroup
